...
int main() {
...
#if FILTERH == 1 and FILTERW == 1
    int H_s[] = {0};
    int H_e[] = {1};
    int W_s[] = {0};
    int W_e[] = {1};
#elif FILTERH == 1 and FILTERW == 3
    int H_s[] = {0};
    int H_e[] = {1};
    int W_s[] = {0};
    int W_e[] = {3};
    ...
    cudaFuncSetAttribute(winograd2DFused_4<bn, bc, bk, FILTERH, FILTERW, 0, 1>, cudaFuncAttributeMaxDynamicSharedMemorySize, maxbytes);
    winograd2DFused_4<bn, bc, bk, FILTERH, FILTERW, 0, 1><<<(BN/bn)*(BK/bk), 256, maxbytes>>>(d_A, tmp_weight_buffer_fused, d_C, kernel_stride_gpu, H_start_gpu, H_end_gpu, W_start_gpu, W_end_gpu, NH, NW, Bi, Hi, Wi, BC, BK, Ho, Wo, PH, PW, num_split);

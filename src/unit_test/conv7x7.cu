...
//    cudaFuncSetAttribute(winograd2DFused_16<bn, bc, bk, 3, 3, 8, 9>, cudaFuncAttributeMaxDynamicSharedMemorySize, maxbytes);
    winograd2DFused_16<bn, bc, bk, 3, 3, 0, 4><<<(BN/bn)*(BK/bk), 256, maxbytes, stream1>>>(d_A, tmp_weight_buffer_fused, d_C, kernel_stride, H_start, H_end, W_start, W_end, NH, NW, Bi, Hi, Wi, BC, BK, Ho, Wo, PH, PW, num_split);
    winograd2DFused_8<bn, bc, bk, 3, 1, 4, 6><<<(BN/bn)*(BK/bk), 256, maxbytes, stream2>>>(d_A, tmp_weight_buffer_fused, d_C+size_C, kernel_stride, H_start, H_end, W_start, W_end, NH, NW, Bi, Hi, Wi, BC, BK, Ho, Wo, PH, PW, num_split);
    winograd2DFused_8<bn, bc, bk, 1, 3, 6, 8><<<(BN/bn)*(BK/bk), 256, maxbytes, stream3>>>(d_A, tmp_weight_buffer_fused, d_C+2*size_C, kernel_stride, H_start, H_end, W_start, W_end, NH, NW, Bi, Hi, Wi, BC, BK, Ho, Wo, PH, PW, num_split);
    winograd2DFused_4<bn, bc, bk, 1, 1, 8, 9><<<(BN/bn)*(BK/bk), 256, maxbytes, stream4>>>(d_A, tmp_weight_buffer_fused, d_C+3*size_C, kernel_stride, H_start, H_end, W_start, W_end, NH, NW, Bi, Hi, Wi, BC, BK, Ho, Wo, PH, PW, num_split);
}
...
//        cudaMemcpy(d_C, h_C, mem_size_C, cudaMemcpyHostToDevice);
    ...
    int H_s2[] = {0, 0, 0, 3, 3, 3, 6, 6, 6};
    int H_e2[] = {3, 3, 3, 6, 6, 6, 7, 7, 7};
    int W_s2[] = {0, 3, 6, 0, 3, 6, 0, 3, 6};
    int W_e2[] = {3, 6, 7, 3, 6, 7, 3, 6, 7};

    cudaMemcpy(H_start_gpu, H_s2, num_split*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(W_start_gpu, W_s2, num_split*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(H_end_gpu, H_e2, num_split*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(W_end_gpu, W_e2, num_split*sizeof(int), cudaMemcpyHostToDevice);

...
#define Hi 1024  //input h
#define Wi 448 //input w
...
__global__ void inputNorm2WinoTransform2D_base(const T *norm_input, T *wino_input, const int *kernel_stride, const int *H_start, const int *H_end, const int *W_start, const int *W_end, int nH, int nW, int B, int H, int W, int C, int pad_h, int pad_w, int N) {
    ...
    int tx = tid % (C * nH * nW * B) % (B * nH * nW) % B; //n
...
__device__ void inputNorm2WinoTransform2D(const float *norm_input, float input_smem[][bc*bn], const int *kernel_stride, const int *H_start, const int *H_end, const int *W_start, const int *W_end, int nH, int nW, int B, int H, int W, int C, int pad_h, int pad_w, int by, int bz, int warp_id, int lane_id, int c_i) {
...
__global__ void winograd2D(const float *input, float *wino_input, const int *kernel_stride, const int *H_start, const int *H_end, const int *W_start, const int *W_end, int nH, int nW, int B, int H, int W, int C, int K, int pad_h, int pad_w)
{
    ...
    for (int i = 0; i < C; i+=bc) {
        ...
        //////// input transform //////
        inputNorm2WinoTransform2D<bn, bc, bk>(input, input_smem, kernel_stride, H_start, H_end, W_start, W_end, nH, nW, B, H, W, C, pad_h, pad_w, by, bz, warp_id, lane_id, i);
        __syncthreads();
        //////////////////////////////
#pragma unroll
        for (int j = 0; j < 16; j++) {
            wino_input[j*C*nH*nW*B+(warp_id+i)*nH*nW*B+by*B+lane_id] = input_smem[j][warp_id*32+lane_id];
        }
        //////// input transform //////
//        inputNorm2WinoTransform2D<bn, bc, bk>(input, wino_input, kernel_stride, H_start, H_end, W_start, W_end, nH, nW, B, H, W, C, pad_h, pad_w, by, bz, warp_id, lane_id, i);
    }
}

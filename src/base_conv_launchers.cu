...
//    }
    ...
    cout << N << endl;
//    inputNorm2WinoTransform2D2 <float> <<<(N - 1 + 512) / 512, 512>>> (input, tmp_input_buffer, kernel_stride_gpu, H_start_gpu, H_end_gpu, W_start_gpu, W_end_gpu, nH, nW, B, H, W, C, pad_h, pad_w, time, N);
...
//    dim3 bDim4(K, 1, 1);
//    dim3 gDim4(nH*nW, B, kernel_size);
//    float *tmp_output = nullptr;
//    cudaMalloc((void**)&tmp_output, num_split*B*output_H*output_W*K*sizeof(float));
